...
- Re-ask gate: after a judgment that neither hinted nor compacted, the next judgment waits until context has grown 20k tokens or 3 more exchanges have completed, whichever comes first. A hint or compaction clears the gate, so a new checkpoint right after a hint is still judged. Replayed against 248 logged judgments (230 Pi, 18 Claude Code; 56 compaction epochs, 43 with a qualifying judgment) it sends 47 % fewer TypeSafe requests and withholds the first qualifying judgment in 4 of those 43 epochs, each estimated one exchange later. That estimate is unverified: in 42 of 43 epochs the qualifying judgment was the last one logged before the person compacted or left. A token-only 20k gate saved 61 % but withheld 9, and on a slow-growing session could stay shut for about 33 exchanges, which is why the exchange count opens it too. Context that shrinks keeps the gate shut until the exchange count opens it.
