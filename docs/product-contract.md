...
- `COMPACT_ADVISER_DISABLE` is a per-session kill switch. Set to `1`, `true`, `yes` or `on` (case-insensitive, surrounding whitespace ignored), it makes the product take no action for that process: no TypeSafe/Jev request, no hint, no automatic compaction, and no status-line output that depends on a judgment. It wins over a saved `hint` or `auto` mode and over every other enablement path. Any other value, including unset, leaves behavior unchanged. Each host reads it once per session, since a session's environment is fixed. Both hosts parse it through a byte-identical `disable.ts`.
- Non-interactive sessions never judge. Pi requires `mode === "tui"` with a UI; the Claude Code mod requires the host's `session.start` `isInteractive` flag. Neither host guesses from `CI` or similar ambient variables.
