...
- Optional TypeSafe request logging is off by default and can be enabled from the compact-adviser settings menu. When on, each request body and the matching Jev response (answers, score, usage, floor, qualifies) are appended to a local jsonl log with existing secret redaction and never the API key.
