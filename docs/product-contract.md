...
- Inspect cheap local state before any Jev request.
- Jev snapshots consider up to the last 64 transcript messages, including tool results, clipped by existing byte budgets (about 14kB recent tail, per-message caps, 8kB user-constraint budget, and a 32kB request refuse path). Message count is not the tight payload bottleneck.
...
- Installing or loading the package is consent to send eligible checkpoint context to OpenRouter when a key is available and other product gates pass (mode, minimum context, idle session, and so on). An OpenRouter API key may be entered in the compact-adviser menu and stored in the same settings store as mode and threshold; after save the UI reports only presence or key source (`env` / `saved` / `.env` / `missing`), never the value. A non-empty launch-environment value wins over the saved key, which wins over a cwd `.env`.
...
The Claude Code mod uses its own `userConfig` options (`mode`, `minContextTokens`, `logRequests`, `openrouterApiKey`) in Claude Code's settings, and its own plugin store for the automatic-mode acknowledgement and per-session cooldowns.
`openrouterApiKey` is hidden from `/config` so the host menu never draws the secret.
