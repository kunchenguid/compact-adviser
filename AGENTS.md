...
- The judge question set, `score()` and `floorFor()` must stay byte-identical in `packages/pi-extension/src/judge.ts` and `packages/claude-mod/lib/judge.ts`; TypeSafe request/response log line builders in each package's `log.ts` must match too. `packages/pi-extension/test/lockstep.test.ts` enforces both. Tune both or neither.
