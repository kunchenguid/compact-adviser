...
- The judge question set, `score()` and `floorFor()` must stay byte-identical in `packages/pi-extension/src/judge.ts` and `packages/claude-mod/lib/judge.ts`; `packages/pi-extension/test/lockstep.test.ts` enforces it. Tune both or neither.
