...
- Claude Code hooks cannot `import "node:fs"` and the plugin-test isolate has no `process`; `$.fs.write` replaces the whole file, so Claude writes one jsonl per session (`~/.claude/compact-adviser-requests-<sessionId>.jsonl`). Pi TypeSafe jsonl writes use `appendFileSync` (O_APPEND) in `packages/pi-extension/src/log.ts`. Claude Code's host check refuses `$.fs` paths under macOS automounts such as `/home`, so test fixtures use a local HOME.
- Claude pane facts the settings pane in `packages/claude-mod/hooks/register.ts` is built on: the arrows and Tab move the focus ring only between plain Buttons (a focused `Select` or `Input` takes the arrows); a redraw keeps the ring at its index, and `autoFocus` applies only when the pane first takes the keyboard, so a view change places the ring with `$.ui.focus` and keys every view's elements apart; Escape hands the keys to the prompt even when a `ui.close` hook refuses the close, so the hook re-opens the pane with `focus`. `claude plugin test` drives Button presses only (no Input submit, no person close, and its `ui.focus` chain skips a test's hooks); `scripts/live-e2e.mjs` presses the real keys.
