...
Known key patterns, the product's own saved TypeSafe key, and obvious sensitive-file results (`.env`, `*.pem`, `id_rsa`, ...) are filtered, but this is **best-effort**, not comprehensive secret detection.
