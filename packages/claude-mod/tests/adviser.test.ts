...
describe("turn-end gates", () => {
  ...
  test("the hint floor slides with context usage: a finished coordinating unit hints only once the window is fuller", async ($, on) => {
    // finished but coordinating scores about 0.5: below the 0.70 floor at 30 % usage,
    // above the 0.40 floor at 85 % usage. Same judgment, different window fill.
    const w = world(on);
    w.respond = async () => ({
      status: 200,
...
describe("commands", () => {
  ...
  test("status reports readiness and the engine threshold, never the key", async ($, on) => {
    ...
    expect(line).toBe(
      "Mode: hint. Minimum: 40,000 tokens. Context: 60,000 (30% of the window; hint floor 0.70). Key: env. No cooldown; semantic checks still apply. Claude Code auto-compacts at 167,000 tokens. Request log: off. Settings: /config (compact-adviser rows) and /compact-adviser.",
    );
...
describe("settings pane", () => {
  ...
  test("reset, status, and close act through the same paths as the commands", async ($, on) => {
    ...
    expect(text(await $.ui.render(pane))).toContain(
      "Mode: off. Minimum: 40,000 tokens. Context: 60,000 (30% of the window; hint floor 0.70). Key: env.",
    );
