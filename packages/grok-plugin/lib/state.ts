// Per-session cooldown facts, the Grok counterpart of the Pi extension's session entries.
// Grok hooks are separate short-lived processes, so these live in one JSON file per session
// under the adviser's own data directory; nothing survives in memory between hook runs.
//
